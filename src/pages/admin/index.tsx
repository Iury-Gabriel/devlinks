    ...
    return (
        <div className="flex items-center flex-col min-h-screen pb-7 px-2">
            ...
            <form className="flex flex-col mt-8 mb-3 w-full max-w-xl" onSubmit={handleRegister}>
                ...
                <button type="submit" className="bg-blue-600 h-9 text-white rounded-md font-medium gap-4 flex justify-center items-center mb-7">
                    Cadastrar
                </button>
            ...
            {links.map((link) => (
                <article key={link.id} className="flex items-center justify-between w-11/12 max-w-xl rounded py-3 px-2 mb-2 select-none" style={{ backgroundColor: link.bg, color: link.color }}>
                    <p>{link.name}</p>
                    <div>
                        <button className="border border-dashed p-2 rounded bg-neutral-900" onClick={() => handleDeleteLink(link.id)}>
